...
export function EventsPageClient({ events, registeredEventIds }: EventsPageClientProps) {
  ...
  return (
    <div className="px-6 py-6 max-w-6xl mx-auto">
      {/* Page header */}
      <div className="mb-5">
        <h1 className="font-display font-black text-[28px]" style={{ color: '#112535' }}>Events</h1>
      </div>
      ...
      <div className="flex items-center gap-2 flex-wrap mb-6">
        {FILTERS.map(f => (
          <button
            ...
            className="font-condensed font-semibold uppercase tracking-[0.1em] text-[11px] rounded-full px-4 py-1.5 transition-all"
            style={{
              backgroundColor: filter === f.key ? '#1b3c5a' : 'transparent',
              color: filter === f.key ? 'white' : '#7a8a96',
              border: `1px solid ${filter === f.key ? '#1b3c5a' : 'rgba(27,60,90,0.15)'}`,
            }}
          >
            {f.label}
          </button>
