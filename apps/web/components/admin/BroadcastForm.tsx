...
const TYPE_META: Record<NotifType, { color: string; bg: string; border: string }> = {
  system_general: { color: 'var(--admin-text)', bg: 'rgba(27,60,90,0.06)',   border: 'rgba(27,60,90,0.15)' },
  ...
  return (
    <div className="grid grid-cols-[1fr_360px] gap-6 items-start">
      ...
      >
        {/* Title */}
        <div className="mb-4">
          <label className="block font-condensed font-bold uppercase tracking-[0.14em] text-[12px] text-[color:var(--admin-text-2)] mb-1.5">
          ...
          />
          <p className="font-condensed text-[12px] text-[color:var(--admin-text-2)] mt-0.5 text-right">{title.length}/100</p>
        </div>
        ...
        <div className="mb-4">
          ...
          <textarea
            ...
            onBlur={e => (e.currentTarget.style.borderColor = 'rgba(27,60,90,0.18)')}
          />
          <p className="font-condensed text-[12px] text-[color:var(--admin-text-2)] mt-0.5 text-right">{message.length}/500</p>
        </div>

        {/* Audience */}
        <div className="mb-4">
          <label className="block font-condensed font-bold uppercase tracking-[0.14em] text-[12px] text-[color:var(--admin-text-2)] mb-2">
            Audience
          </label>
          <div className="flex flex-col gap-2">
            {AUDIENCE_OPTIONS.map(opt => (
              <label key={opt.value} className="flex items-center gap-3 cursor-pointer">
                ...
                />
                <div>
                  <span className="font-condensed font-semibold text-[12px] text-[color:var(--admin-text-strong)]">{opt.label}</span>
                  <span className="font-condensed text-[12px] text-[color:var(--admin-text-2)] ml-2">{opt.desc}</span>
                </div>
              </label>
            ))}
          </div>
        ...
        <div className="mb-4">
          ...
          <select
            ...
            style={{ border: '1px solid rgba(27,60,90,0.18)', color: 'var(--admin-text-strong)', backgroundColor: 'var(--admin-card)' }}
          >
        ...
        <div className="mb-6">
          <label className="block font-condensed font-bold uppercase tracking-[0.14em] text-[12px] text-[color:var(--admin-text-2)] mb-1.5">
            Action URL <span className="text-[color:var(--admin-text-2)]">(optional)</span>
          </label>
          <input
            type="text"
            value={actionUrl}
      ...
      {/* Preview */}
      <div>
        <p className="font-condensed font-bold uppercase tracking-[0.16em] text-[12px] text-[color:var(--admin-text-2)] mb-3">
          Live Preview
        </p>
        <div
        ...
        >
          {previewValid ? (
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="font-condensed font-bold text-[12px] text-[color:var(--admin-text-2)]">just now</span>
                  </div>
                  <p className="font-body font-semibold text-[13px] text-[color:var(--admin-text-strong)] mb-0.5">{title}</p>
                  <p className="font-body text-[12px] text-[color:var(--admin-text-2)]">
                    {message.split(/\*\*(.*?)\*\*/g).map((part, i) =>
                      i % 2 === 1 ? <strong key={i} className="text-[color:var(--admin-text)]">{part}</strong> : part,
                    )}
                  </p>
                </div>
            ...
            <div className="p-6 text-center">
              <p className="font-condensed text-[12px] text-[color:var(--admin-text-2)]">Fill in title and message to see preview</p>
            </div>
          )}
        </div>
        ...
        >
          <p className="font-condensed font-bold uppercase tracking-[0.14em] text-[12px] text-[color:var(--admin-text-2)] mb-1">Audience</p>
          <p className="font-condensed text-[12px] text-[color:var(--admin-text)] font-semibold">
