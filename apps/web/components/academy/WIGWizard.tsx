...
export function WIGWizard({ courseId, domain: defaultDomain = 'professional' }: Props) {
  ...
  return (
    <div style={{ backgroundColor: '#111926', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '8px', padding: '24px' }}>
      ...
      {loading ? (
        <p style={{ color: 'rgba(250,249,247,0.2)', fontSize: '12px', fontFamily: 'var(--font-condensed), sans-serif', letterSpacing: '0.1em', textTransform: 'uppercase' }}>Loading…</p>
      ) : showWizard ? (
        <WizardForm
          domain={activeDomain}
          wizard={w}
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '24px' }}>
            ...
            <div key={n} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <div style={{
                ...
                {done
                  ...
                  : <span style={{ fontFamily: 'var(--font-condensed), sans-serif', fontWeight: 700, fontSize: '12px', color: active ? '#0A0F18' : 'rgba(250,249,247,0.3)' }}>{n}</span>
                }
              </div>
        ...
        <span style={{ fontFamily: 'var(--font-condensed), sans-serif', fontWeight: 700, fontSize: '12px', letterSpacing: '0.12em', textTransform: 'uppercase', color: BLUE, marginLeft: '8px' }}>
      ...
      {w.step === 4 && (
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '14px', flexWrap: 'wrap' }}>
            ...
            {isEditing && (
              <button type="button" onClick={onCancelEdit} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-condensed), sans-serif', fontWeight: 700, fontSize: '12px', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(250,249,247,0.25)', padding: 0 }}>
                Cancel
              </button>
            )}
          </div>
  ...
  return (
    <div style={{ backgroundColor: `${color}0A`, border: `1px solid ${color}22`, borderRadius: '8px', padding: '20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
        <p style={{ fontFamily: 'var(--font-condensed), sans-serif', fontWeight: 700, fontSize: '12px', letterSpacing: '0.2em', textTransform: 'uppercase', color, margin: 0 }}>
          Wildly Important Goal · Saved {updatedDate}
        </p>
        <button type="button" onClick={onEdit} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-condensed), sans-serif', fontWeight: 700, fontSize: '12px', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(250,249,247,0.3)', padding: 0 }}>
          Edit
        </button>
      </div>
      ...
      <div className="academy-tool-grid-3" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
        {[['FROM', c.from], ['TO', c.to], ['BY', c.by ? formatDate(c.by) : '—']].map(([k, v]) => (
          <div key={k}>
            <p style={{ fontFamily: 'var(--font-condensed), sans-serif', fontWeight: 700, fontSize: '12px', letterSpacing: '0.2em', textTransform: 'uppercase', color, margin: '0 0 4px' }}>{k}</p>
