...
export function EventReminderEmail({ displayName, event }: EventReminderProps) {
  ...
  return (
    <Html>
      <Head />
      <Preview>Tomorrow: {event.title} — don't miss it</Preview>
      <Body style={bodyStyle}>
        ...
        <Container style={containerStyle}>
          <Heading style={headingStyle}>Tomorrow.</Heading>

          <Text style={textStyle}>
            {firstName}, your event starts in 24 hours. Here's what you need to know.
          </Text>
          ...
          {event.zoomUrl && (
            <Section style={{ textAlign: 'center', margin: '24px 0' }}>
              <Button href={event.zoomUrl} style={ctaStyle}>Join Tomorrow →</Button>
            </Section>
          )}

          <Section style={{ textAlign: 'center', margin: '16px 0' }}>
            <Button href={`${appUrl}/events/${event.id}`} style={secondaryCtaStyle}>
              View Event Details
            </Button>
          </Section>

          <Text style={footerNoteStyle}>
            Show up. Be present. This is where the work happens.
          </Text>
        </Container>

        <Section style={footerStyle}>
          <Text style={footerTextStyle}>© {new Date().getFullYear()} Evolved Pros · evolvedpros.com</Text>
          <Text style={footerTextStyle}>You're receiving this because you registered for this event.</Text>
        </Section>
      </Body>
    </Html>
  )
...
const footerNoteStyle: React.CSSProperties = { fontSize: '13px', color: '#7a8a96', margin: '24px 0 0', textAlign: 'center', fontStyle: 'italic' }
