...
export default function OnboardPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data: { user } }) => {
      ...
      supabase
        ...
        .single()
        .then(({ data }) => {
          const name = data?.full_name?.split(' ')[0] ?? data?.display_name ?? 'there'
          setDisplayName(name)
        })
    })
    // Pre-fetch general channel ID for step 3
    supabase
      .from('channels')
      .select('id')
      .eq('slug', 'general')
      .single()
      .then(({ data }) => { if (data) setGeneralChannelId(data.id) })
  }, [])
  ...
  async function handleIntroPost() {
    ...
    if (trimmed.length < 10 || !generalChannelId) { setStep(4); return }
    setPosting(true)
    setPostError(null)
    ...
    if (!res.ok) {
      ...
      setPostError(data.error ?? 'Post failed — you can introduce yourself in the community later.')
    }
    setPosting(false)
    setStep(4)
  }
