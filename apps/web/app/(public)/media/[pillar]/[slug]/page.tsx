...
export default async function StoryPage({ params }: { params: { pillar: string; slug: string } }) {
  const story = await fetchStory(params.pillar, params.slug)
  if (!story) notFound()
  if (story.story_type === 'redirect' && story.source_url) redirect(story.source_url)
...
<<<<<<< HEAD
      ...
      <header className="flex items-center justify-between px-6 py-3" style={{ backgroundColor: '#fff', borderBottom: '1px solid rgba(10,15,24,0.08)' }}>
        <Link href="/media" className="flex items-center gap-2" style={{ textDecoration: 'none' }}>
          <span className="font-condensed font-bold tracking-[0.18em] text-[13px]" style={{ color: '#0A0F18' }}>EVOLVED<span style={{ color: '#C9302A' }}>&middot;</span>PROS</span>
...
<<<<<<< HEAD
        <p className="font-condensed font-bold uppercase tracking-[0.12em] text-[10px] mb-2" style={{ color: pColor }}>{pLabel}</p>
        <h1 className="font-bold text-[28px] sm:text-[34px] leading-tight mb-4" style={{ color: '#0A0F18', fontFamily: 'Georgia, serif' }}>{story.title}</h1>
        {story.excerpt && <p className="font-body text-[15px] leading-relaxed mb-5" style={{ color: 'rgba(10,15,24,0.6)' }}>{story.excerpt}</p>}
=======
...
>>>>>>> origin/claude/init-evolved-pros-platform-Q2oUw
        <div className="flex items-center gap-3 mb-8 pb-6" style={{ borderBottom: '1px solid rgba(10,15,24,0.08)' }}>
          ...
          <div>
<<<<<<< HEAD
            <p className="font-body font-semibold text-[13px]" style={{ color: '#0A0F18' }}>{story.author ?? 'George Leith'}</p>
            <p className="font-condensed text-[10px]" style={{ color: 'rgba(10,15,24,0.4)' }}>{formatDate(story.published_at)} &middot; {minutes} min read</p>
          </div>
        </div>
        ...
        {story.story_type === 'pioneer_spin' && story.source_name && (
          <div className="rounded-lg px-4 py-3 mb-8 flex items-center gap-2" style={{ backgroundColor: 'rgba(10,15,24,0.03)', border: '1px solid rgba(10,15,24,0.06)' }}>
            <span className="font-condensed font-bold uppercase tracking-[0.1em] text-[9px]" style={{ color: 'rgba(10,15,24,0.35)' }}>Pioneer Driver</span>
            <span className="font-body text-[12px]" style={{ color: 'rgba(10,15,24,0.5)' }}>
              Inspired by{' '}
              {story.source_url ? <a href={story.source_url} target="_blank" rel="noopener noreferrer" className="underline" style={{ color: pColor }}>{story.source_name}</a> : story.source_name}
            </span>
          </div>
        )}
        <div className="prose prose-stone max-w-none font-body text-[15px] leading-[1.8]" style={{ color: 'rgba(10,15,24,0.8)' }} dangerouslySetInnerHTML={{ __html: html }} />
        {story.tags.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-10 pt-6" style={{ borderTop: '1px solid rgba(10,15,24,0.06)' }}>
            {story.tags.map(tag => (
              <span key={tag} className="font-condensed font-semibold uppercase tracking-[0.1em] text-[9px] px-2.5 py-1 rounded" style={{ backgroundColor: 'rgba(10,15,24,0.04)', color: 'rgba(10,15,24,0.45)' }}>{tag}</span>
=======
...
<<<<<<< HEAD
      {(related ?? []).length > 0 && (
        <section className="max-w-3xl mx-auto px-4 sm:px-6 pb-12">
          <h2 className="font-condensed font-bold uppercase tracking-[0.16em] text-[10px] mb-4 pl-4" style={{ color: 'rgba(10,15,24,0.35)', borderLeft: `4px solid ${pColor}` }}>More in {pLabel}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {(related ?? []).map((r: any) => (
              <Link key={r.id} href={`/media/${r.pillar}/${r.slug}`} className="block group bg-white rounded-lg overflow-hidden" style={{ border: '1px solid rgba(10,15,24,0.06)', textDecoration: 'none' }}>
                {r.featured_image_url && <div className="overflow-hidden" style={{ height: 100 }}><img src={r.featured_image_url} alt="" className="w-full h-full object-cover" /></div>}
                <div className="p-3">
                  <p className="font-condensed font-bold uppercase tracking-[0.12em] text-[8px] mb-1" style={{ color: pColor }}>{pLabel}</p>
                  <h3 className="font-body font-semibold text-[13px] leading-snug group-hover:text-[#C9302A] transition-colors" style={{ color: '#0A0F18' }}>{r.title}</h3>
                  <p className="font-condensed text-[9px] mt-1" style={{ color: 'rgba(10,15,24,0.35)' }}>{readTime(r.body)} min read</p>
=======
...
<<<<<<< HEAD
      <div className="max-w-3xl mx-auto px-4 sm:px-6 pb-12">
        <div className="rounded-xl px-8 py-8 text-center" style={{ backgroundColor: '#0A0F18' }}>
          <p className="font-condensed font-bold uppercase tracking-[0.2em] text-[10px] mb-2" style={{ color: '#C9A84C' }}>Go deeper</p>
          <h3 className="font-bold text-[18px] mb-2" style={{ color: '#F7F4EF', fontFamily: 'Georgia, serif' }}>The Academy puts the system behind the headlines.</h3>
          <p className="font-body text-[13px] mb-4" style={{ color: 'rgba(245,240,232,0.5)' }}>VIP gets Pillars 1\u20133. Professional gets the full 6-pillar EVOLVED system.</p>
