...
export async function GET() {
  ...
  const { data, error } = await supabase
    .from('events')
    .select('id, title, event_type, starts_at, ends_at, required_tier, registration_count, is_published, recording_url, zoom_url, description')
...
export async function POST(request: Request) {
  ...
  if (error || !data) return NextResponse.json({ error: 'Failed to create event' }, { status: 500 })
